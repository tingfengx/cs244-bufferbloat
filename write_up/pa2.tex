...
\ul{How does the RTT reported by ping vary with the queue size? Write a symbolic equation to describe the relation between the two. Ignore the computation overheads in ping that might affect the final result.)}
...
\begin{figure}[H]
	\center\includegraphics[width=0.8\textwidth]{plot/fit}
	\captionsetup{format=hang}
	\caption{\label{fig:rtt q relationship}Scatter plot of RTT (in ms) and the max queue size used respectively. Plotted with the fitted line via linear regression. }
\end{figure}
