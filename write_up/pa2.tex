...
Notice that here (as shown in Table \ref{tab:mean and std}) when queue size is 20, we have the lowest mean and standard deviation in the fetch time. While when buffer size is 100, we have the worst performance, both in terms of the mean fetch time and the standard deviation. When buffer size is 5, the performance is in between the other two experiments. I will explain why we are seeing this counter-intuitive\footnote{When we add a buffer, it is more intuitive to think that more buffer means better performance, or at least it should not hurt the performance. However, this is not the case with router buffers, and this is counter-intuitive. } case in my ``Discussion'' section below.
...
\begin{figure}[H]
	\center\includegraphics[width=0.9\textwidth]{../fit}
	\captionsetup{format=hang}
